...
def calStockTradingPNL() -> float:
    trading_data = pd.read_csv("302.csv", encoding="gbk", converters={"证券代码": str})
    trading_data = trading_data[["证券代码", "证券名称", "成交结果", "成交价格", "成交数量"]]
    trading_data.columns = ["code", "stock_name", "direction", "price", "quantity"]
    trading_data.code = ['0' * (6 - len(str(code))) + code for code in list(trading_data.code)]
    trading_data.code = zcodeparse(list(trading_data.code), formatlike="600000.SH")
    sql = \
        '''
        SELECT
        ''' + '''
    ...
    trading_data = pd.merge(trading_data, stock_price, left_on="code", right_on="S_INFO_WINDCODE")
    ...
    trading_data["PNL"] = trading_data.S_DQ_CLOSE.sub(trading_data.price).mul(trading_data.quantity).mul(
        trading_data.direction).sub(trading_data.trading_fee)
...
def calFutureTradingPNL() -> float:
    future_data = pd.read_csv("83023052.csv", encoding="gbk")
